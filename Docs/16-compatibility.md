...
- **Shadows are gone.** The projected shadows did not render properly and were taken out whole: `MarinaStyle.Shadows` and the `ShadowStyle` type, `ShadowProjection`, the `RenderLayerKind.StructureShadows` and `BerthShadows` layers, the `RenderPass.Shadow` pass and the `RenderAnimation.Unlit` flag. `RenderPass.Transparent` is now 1 (it was 2); the other layer kinds keep their numbers. A backend that drew a shadow pass drops it, and a copy of `marinaWebGL.js` kept by a host must be replaced with the current one. A marina file written before still opens: its `presentation.shadows` section is read and dropped, not written back.
