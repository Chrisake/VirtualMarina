...
| `Docks` | `Dock` records |
| `Slips` | `Slip` records, each referencing a dock by `DockId` |
...
| `LandAreas` | Quays, breakwaters and lawns drawn for context |
...
var layout = new MarinaLayoutBuilder("VirtualMarina Harbor")
    .AddLandArea(new LandArea("quay", new OrientedRect(new Vector2(0, -19), new Vector2(260, 26), 0), 1.0f, LandKind.Quay))
...
| `PilingSpacing` | Column spacing (concrete) or half the guide-pile spacing (floating); default 6 m |
| `WithCenter(center)` | Copy moved to a new center |

| `DockType` | Rendering | Default deck height |
|---|---|---|
| `FloatingWooden` (default) | Plank deck with seams, walers, dark pontoon floats, wooden guide piles | 0.5 m |
...
| Member | Meaning |
|---|---|
| `new Slip(id, dockId, center, headingDegrees, length, width)` | A Free slip |
...
`new LandArea(id, OrientedRect area, float height, LandKind kind = Quay)`. `LandKind` is `Quay` (light concrete), `Breakwater` (gray) or `Grass` (green). Land is drawn only and isn't interactive.
...
| `GetDock`, `GetDocks` | `GetDivider`, `GetDividers`, `GetDividersByDock` | `GetSlip`, `GetSlips`, `GetSlipsByDock`, `GetSlipsByStatus` |
